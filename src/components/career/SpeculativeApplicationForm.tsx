...
const SpeculativeApplicationForm = () => {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.desiredRole.trim()) {
      newErrors.desiredRole = 'Please describe your desired role';
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log('Speculative application submitted:', formData);
    
    // Reset form
    setFormData({ name: '', email: '', desiredRole: '' });
    setIsSubmitting(false);
    
    // Show success message (you can implement toast here)
    alert('Thank you for your interest! We\'ll be in touch if a suitable opportunity arises.');
  };
          ...
          <Card className="bg-card/80 backdrop-blur-sm border-2 border-coresync-primary-light/20">
            ...
            <CardContent>
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-2">
                  <Label htmlFor="desiredRole">Desired Role & Experience *</Label>
                  <Textarea
                    id="desiredRole"
                    name="desiredRole"
                    value={formData.desiredRole}
                    onChange={handleInputChange}
                    placeholder="Tell us about the role you're interested in and your relevant experience..."
                    rows={4}
                    className={errors.desiredRole ? 'border-red-500' : ''}
                  />
                  {errors.desiredRole && (
...
export default SpeculativeApplicationForm;
